    ...
    axios
        ...
        .then((res) => {
            ...
            e.offsetParent.getElementsByTagName(
                ...
                .split("/")
                .slice(0, 3)
                .join("/")}/storage/${res.data.path}`;
            e.offsetParent.getElementsByTagName(
                "a"
            )[0].textContent = `view file`;
            e.offsetParent.querySelector(
                `input[name='${e.dataset.name}']`
            ).value = res.data.path;

            e.offsetParent.getElementsByClassName(
                "progress-bar"
            )[0].style.width = `100%`;
        ...
        .catch((err) => {
            ...
            if (Array.isArray(err.response.data.file)) {
                e.offsetParent.getElementsByClassName(
                    "progress-bar"
                )[0].textContent = `${err.response.data.file[0]}`;
            } else {
                e.offsetParent.getElementsByClassName(
                    "progress-bar"
                )[0].textContent = `${err.response.statusText}`;
            }
        });
};
...
var uploadFileEquipment = async (e) => {
    ...
    axios
        ...
        .then((res) => {
            ...
            e.offsetParent.offsetParent.querySelector(
                `img`
...
var setVisible = (visible) => {
    ...
    document.querySelector("#loading").style.display = visible
        ? "block"
        : "none";
};
/**
...
var pageDisable = (elements = "") => {
    let inner_btn = document
        .getElementsByClassName("app-main__inner")[0]
        .querySelectorAll(elements === "" ? `button,select,input` : elements);
...
var findNameUser = (user) => {
    return (
        user.translations.find((item) => item.locale === locale) ??
        user.translations[0]
    );
};
...
var setCookie = (cname, cvalue, exdays, path) => {
    const d = new Date();
