...
var render_html = () => {
    // create tr in table
    let tables = document
        .getElementById("group-table")
        .getElementsByClassName("table");
    for (let i = 0; i < tables.length; i++) {
        const table = tables[i];
        let reduce = 0;
        let reduce_hod = 0;
        let temp_rules = evaluateForm.detail.filter(
            (value) => value.rules.categorys.name === table.id.substring(6)
        );
        ...
        if (admin && temp_rules.length > 0) {
            let cellHead = table.tHead.firstElementChild.cells[0];
            let btn_ad = document.createElement("button");
            btn_ad.innerHTML = "Add";
            btn_ad.setAttribute("data-toggle", "modal");
            btn_ad.setAttribute("data-target", "#add-rule-modal");
            btn_ad.setAttribute("data-group", temp_rules[0].rules.category_id);
            cellHead.appendChild(btn_ad);
        }
        ...
        for (let index = 0; index < temp_rules.length; index++) {
            ...
            try {
                ...
                if (admin) {
                    ...
                    btn_change.setAttribute(
                        "data-target",
                        "#switch-rule-modal"
                    );
                    btn_change.setAttribute(
                        "data-group",
                        element.rules.category_id
                    );
                    btn_change.setAttribute("data-id", element.id);
                    cellIndex.appendChild(btn_change);
                } else {
                ...
                cellBaseLine.appendChild(
                    newInput(
                        "number",
                        className,
                        "base_line",
                        element.base_line.toFixed(2),
                        "",
                        `changeValue(this)`,
                        !operation
                    )
                ...
                cellTarget.appendChild(
                    newInput(
                        "number",
                        className,
                        "target",
                        element.target.toFixed(2),
                        "",
                        `changeValue(this)`,
                        !operation
                    )
                ...
                cellTargetPC.textContent =
                    findTargetPercent(element, temp_rules).toFixed(2) + `%`;

                let cellActual = newRow.insertCell();
                cellActual.appendChild(
                    newInput(
                        "number",
                        className,
                        "actual",
                        element.actual.toFixed(2),
                        "",
                        `changeValue(this)`,
                        !operation
                    )
                ...
                cellActualPC.textContent =
                    findActualPercent(element, temp_rules).toFixed(2) + `%`;
        ...
        if (temp_rules.length > 0) {
            ...
            let reduce_hod_input =
                table.offsetParent.firstElementChild.lastElementChild.querySelectorAll(
                    "input"
                )[0];
            let reduce_input =
                table.offsetParent.firstElementChild.lastElementChild.querySelectorAll(
        ...
        head_weight =
            table.offsetParent.firstElementChild.querySelector("input");
        if (head_weight && head_weight.name in template) {
            head_weight.value = template[head_weight.name].toFixed(2);
        }

        let sum_weight = temp_rules.reduce(
            (total, cur) => total + cur.weight,
            0.0
        );
        let sum_cal =
            temp_rules.reduce((total, cur) => total + cur.cal, 0.0) -
            (reduce + reduce_hod);
        table.tFoot.lastElementChild.cells[5].textContent = `${sum_weight.toFixed(
            2
        )}%`;
        table.tFoot.lastElementChild.cells[11].textContent = `${sum_cal.toFixed(
            2
        )}%`;
        summary.push({
    ...
    for (let index = 0; index < summary_table.tBodies[0].rows.length; index++) {
        ...
        row.cells[2].textContent = `${(
            (summary[index].cal * summary[index].weight) /
            100
        ).toFixed(2)} %`;
        setAttributes(row.cells[2], {
            "data-toggle": "tooltip",
            title: `(${summary[index].cal.toFixed(2)} * ${summary[
                index
            ].weight.toFixed(2)}) / 100`,
            "data-placement": "top",
        });
    }
    summary_table.tFoot.rows[0].cells[2].textContent = `${total.toFixed(2)} %`;
    summary_table.tFoot.rows[0].cells[1].textContent = `${sum_weight.toFixed(
        2
    )} %`;
...
const changeValue = (e) => {
    ...
    evaluateForm.detail.forEach((element, key) => {
        if (
            e.offsetParent.parentNode.cells[1].textContent ===
            element.rules.name
        ) {
            ...
            let rule = evaluateForm.detail[key],
                ...
                sumary_index = summary.findIndex(
                    (item) => item.category === rule.rules.categorys.name
                );
            ...
            if (rule.rules.calculate_type !== null) {
                ...
                tr.cells[11].dataset.originalTitle = changeTooltipCal(
                    tr.cells[column + 2].dataset.originalTitle,
                    rule
                );
                rule.cal = cal;
                ...
                let sumCal = evaluateForm.detail
                    .filter(
                        (item) =>
                            item.rules.categorys.name ===
                            rule.rules.categorys.name
                    )
                    .reduce(
                        (total, currentValue) => total + currentValue.cal,
                        0
                    );
                ...
                let sumAch = evaluateForm.detail
                    ...
                    .reduce(
                        (total, currentValue) => total + currentValue.ach,
                        0
                    );

                // foot.lastElementChild.cells[10].textContent = parseFloat(sumAch).toFixed(2) + '%'
                foot.lastElementChild.cells[11].textContent =
                    parseFloat(sumCal).toFixed(2) + "%";
                ...
                for (
                    let index = 0;
                    index < table.tBodies[0].rows.length;
                    index++
                ) {
                    const element = table.tBodies[0].rows[index];
                    sum_weight += summary[index].weight;
                    let calculator =
                        (summary[index].weight * summary[index].cal) / 100;
                    element.cells[2].textContent = `${parseFloat(
                        calculator
                    ).toFixed(2)} %`;
                    total += parseFloat(calculator);
                    element.cells[2].dataset.originalTitle = `(${summary[
                        index
                    ].cal.toFixed(2)} * ${summary[index].weight.toFixed(
                        2
                    )}) / 100`;
                }
                table.tFoot.rows[0].cells[1].textContent = `${sum_weight.toFixed(
                    2
                )} %`;
                table.tFoot.rows[0].cells[2].textContent = `${total.toFixed(
                    2
                )} %`;
            }
        }
...
const submit = () => {
    setVisible(true);
    putEvaluateSelf(evaluate.id, evaluateForm)
        .then((res) => {
            if (res.status === 201) {
...
const submitToManager = () => {
    ...
    putEvaluateSelf(evaluate.id, evaluateForm)
        .then((res) => {
            let label_status = document
                .getElementsByClassName("card-header")[0]
                .querySelector("span");
            if (res.status === 201) {
                label_status.textContent = res.data.data.status;
                if (
                    res.data.data.status !== status.READY &&
                    res.data.data.status !== status.DRAFT
                ) {
                    pageDisable();
                }
                toast(res.data.message, res.data.status);
...
// modal method
$("#add-rule-modal").on("show.bs.modal", async function (event) {
    ...
    try {
        ...
        if (rules.status === 200) {
            let select = modal.find(".modal-body #add_rule_name")[0];
...
$("#add-rule-modal").on("hide.bs.modal", function (event) {
    var button = $(event.relatedTarget); // Button that triggered the modal
    var modal = $(this);
    // var group = button.data('group') // Extract info from data-* attributes
    // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
    // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
    removeAllChildNodes(modal.find(".modal-body #add_rule_name")[0]);
...
const changerule = async () => {
    ...
    try {
        ...
        if (result.status === 200) {
            ...
            document
                .getElementById("switch-rule-modal")
                .querySelector(".close")
                .click();
        }
        // console.log(result)
...
const addrule_new = async () => {
    let r_id = $("#add_rule_name").val();
    let form = {
        form_id: evaluateForm.id,
        new_rule: r_id
    };
    try {
        let result = await postRuleInEvaluate(form);
        if (result.status === 200) {
            toast(result.data.message, result.data.status);
            document
                .getElementById("switch-rule-modal")
        ...
        .then((res) => {
            if (res.status === 200) {
                ...
                for (let index = 0; index < rule.length; index++) {
                    ...
                    select.add(
                        new Option(element.name, element.id, false, false)
                    );
                }
            }
        })
...
const addRule = (e) => {
    ...
    let value = rule.find(
        (element) => element.id === parseInt(rule_name.value)
    );
...
const selectToRemove = (e) => {
    ...
    if (!e.checked) {
        if (evaluateForm.remove.length > 0) {
            evaluateForm.remove.splice(
                evaluateForm.remove.indexOf(parseInt(e.id)),
                1
            );
            parseInt(e.id);
        }
...
const removeInSelected = (e) => {
    evaluateForm.detail = evaluateForm.detail.filter(
        (item) => !evaluateForm.remove.includes(item.rule_id)
    );
    render_html();
};
