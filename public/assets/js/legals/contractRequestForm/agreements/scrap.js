    ...
    try {
        await getComercialLists(document.getElementById('contract_id').value).then(result => {
            if (result.data.length < 1) {
                ...
                // if (!document.getElementById('form-scrap').checkValidity()) {
                    // toast('Can’t find purchase', 'error')
                // }else{
        ...
        if (onSubmit && document.getElementById('form-scrap').checkValidity()) {
            document.getElementById('form-scrap').submit()
        }
        toastClear()
    }
}
