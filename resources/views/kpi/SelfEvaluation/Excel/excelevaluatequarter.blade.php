...
<table>
    ...
    <tfoot>
        <tr>
            ...
            $sum = $group->sum('cal') - ($reduce + ($reduce_hod / 3));
            @endphp
...
<table>
    ...
    <tbody>
        ...
        $result = $item->sum('cal');
        if ($i === 'kpi') {
            $result = $result - ($evaluate->kpi_reduce + ($evaluate->kpi_reduce_hod / 3));
        }
        if ($i === 'key-task') {
            $result = $result - ($evaluate->key_task_reduce + ($evaluate->key_task_reduce_hod / 3));

        }
        if ($i === 'omg') {
            $result = $result - ($evaluate->omg_reduce + ($evaluate->omg_reduce_hod / 3));
        }
